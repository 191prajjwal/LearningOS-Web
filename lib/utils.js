...
export function getLectureProgress(lec) {
  ...
  // Basic industry standard: > 95% is considered complete
  const pct = lec.duration > 0 && lec.last_position > 0
    ? Math.round((lec.last_position / lec.duration) * 100)
    : 0;
    
  if (pct >= 95) return { status: "completed", pct: 100 };
  if (pct > 0 || lec.last_position > 0) return { status: "inprogress", pct: Math.min(pct, 99) };
